...
function showQuestion() {
  ...
  document.getElementById("question-counter").textContent =
    `Soru ${currentIndex + 1}/${TOTAL_QUESTIONS}`;
  ...
  question.options.forEach((option, i) => {
    ...
    letterEl.textContent = OPTION_LETTERS[i];

    const textEl = document.createElement("div");
    textEl.className = "option__text";
...
function handleAnswer(selectedIndex) {
  ...
  submittedAnswers.push({ questionId: question.id, selectedIndex });
  ...
  if (isCorrect) {
    ...
    document.getElementById("feedback-text").textContent =
      `Yanlış! Doğru cevap: ${question.options[question.correctAnswer]}`;
  }
...
function updateRemainingQuestions() {
  const usedIds = new Set(
    currentQuestions.slice(0, currentIndex + 1).map((q) => q.id),
  );
